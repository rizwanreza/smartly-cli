...
| `--dry-run` | bool | Show the generated command, and what the current mode would have done with it, without asking or running it. |
...
the configured `execution.mode` is read.

<Callout type="warn" title="Without --dry-run or --confirm, the command runs.">
  The default `execution.mode` is `auto`. Set `confirm` to be asked every time,
  or `confirm-destructive` to be asked on anything smartly doesn't recognise as
  safe. See [Execution and safety](/docs/execution-and-safety/).
</Callout>
...
| `execution.mode` | `auto`, `confirm`, `confirm-destructive` | `auto` |
...
- Targets bash and zsh on Linux and macOS. Windows and PowerShell are not
  supported yet.
- The classifier behind `confirm-destructive` is a best-effort seatbelt, not a
  sandbox. False negatives are possible. See [What counts as
  destructive](/docs/execution-and-safety/#what-counts-as-destructive).
