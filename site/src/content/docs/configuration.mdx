...
<Callout type="warn" title="`execution.mode: auto` is the default.">
  ...
  confirmation prompt, including destructive ones. `execution.mode: confirm` is
  the one-line change that puts a `[y/N]` gate in front of every command. See
  [Execution and safety](/docs/execution-and-safety/).
</Callout>
